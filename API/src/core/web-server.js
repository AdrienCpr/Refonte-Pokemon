  ...
  async _initializeWebSocket() {
    ...
    function setResult(playerIndex, opponentIndex, RoomFull) {
      ...
      if (actionTurnPlayer === 'attack') {
        ...
        let attack_domage_total = Math.round((opponentCards[opponentPokemonIndex].defense / 3)) - (pokemonTurnPlayer.attack*caluculate_domage_per_type)
        if (attack_domage_total < 0) {
          opponentCards[opponentPokemonIndex].HP = opponentCards[opponentPokemonIndex].HP + attack_domage_total
        }

        io.to(RoomFull).emit('log', attack_domage_total)
